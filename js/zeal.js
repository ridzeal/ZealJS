...
function Zeal() {
	...
	this.gen = {
		...
		table: {
			...
			master: function(id, cols, crud) {
				if (!crud && !crud.create && !crud.read && !crud.update && !crud.delete) {
					return false;
				} else {
		...
		element: {
			...
			text: function(id, config) {
				...
				el.attr("name", (config.name) ? config.name : id);
			...
			select: function(id, optValue, config) {
				...
				// Additional Attributes
				if (config.attributes) {
					for (var j in config.attributes) {
						el.attr(j, config.attributes[j]);
					}
				}
	...
	this.makeTableRes = function(tableId) {
		...
		this.inner2select(newTableId + "Col1", newTableId + "Col1Select", optVal, 0);
		this.inner2select(newTableId + "Col2", newTableId + "Col2Select", optVal, 1);
